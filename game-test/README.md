...
1. **Picks a version.** It reads Mojang's [version manifest](https://piston-meta.mojang.com/mc/game/version_manifest_v2.json) and the [Paper API](https://fill.papermc.io/v3/projects/paper). `latest` resolves to the newest release whose latest Paper build is `STABLE`.
...
3. **Starts the client.** [PortableMC](https://github.com/theorzr/portablemc) (a pinned version, checked against its SHA-256) installs the vanilla client and joins the server with Quick Play. No Microsoft account is needed.
...
A scenario is a JSON array of actions:
...
```sh
uv run --no-project python -B -m unittest discover -s game-test/scripts -p 'test_*.py'
```

On GitHub Actions, `.github/workflows/game_test.yml` runs this on every pull request. You can also start it manually and pick a version and a scenario.
...
| `scripts/scenario.py` | Parses and validates scenarios (no side effects) |
...
| `scripts/versions.py` | Resolves the version from Mojang's manifest and the Paper API |
