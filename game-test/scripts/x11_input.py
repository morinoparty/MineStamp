...
class MinecraftWindow:
    """1つの Minecraft ウィンドウに対する入力操作をまとめたクラス。"""

    def __init__(self, xdotool: str, window_id: int):
        self._xdotool = xdotool
        self.window_id = window_id

    @classmethod
    def wait_for(cls, timeout: float) -> "MinecraftWindow":
        """Minecraft のウィンドウが表示されるまで待ち、見つかったウィンドウを返す。"""
        if not os.environ.get("DISPLAY"):
            raise InputError("DISPLAY is not set; run inside xvfb-run")
        ...
        while time.monotonic() < deadline:
            for option, query in WINDOW_QUERIES:
                ids = _run(xdotool, "search", "--onlyvisible", option, query, allow_no_match=True).split()
                if ids:
                    # 同じ Xvfb 上には1クライアントしか起動しないので、最後に作られたものを使う
                    return cls(xdotool, int(ids[-1]))
            time.sleep(0.5)
        raise InputError(f"Minecraft window did not appear within {timeout:.0f} seconds")

    def focus(self) -> None:
        """キーボードフォーカスがこのウィンドウに無ければ移す。"""
        if self._focused_window() == self.window_id:
            return
        _run(self._xdotool, "windowraise", str(self.window_id))
        _run(self._xdotool, "windowfocus", "--sync", str(self.window_id))
        if self._focused_window() != self.window_id:
            raise InputError("could not give keyboard focus to the Minecraft window")
    ...
    def press_key(self, keysym: str) -> None:
        ...
        _run(self._xdotool, "key", "--clearmodifiers", "--delay", "100", str(unmodified_keycode(keysym)))

    def type_text(self, text: str) -> None:
        """文字列をキー入力する。記号の Shift 等は xdotool が自動で付与する。"""
        self.focus()
        _run(self._xdotool, "type", "--clearmodifiers", "--delay", "50", "--", text)

    def _focused_window(self) -> int:
        return int(_run(self._xdotool, "getwindowfocus", "-f").strip())


def _run(xdotool: str, *arguments: str, allow_no_match: bool = False) -> str:
    """xdotool を実行して標準出力を返す。"""
    try:
...
def unmodified_keycode(keysym: str) -> int:
    ...
    if not display:
        raise InputError("could not open DISPLAY to resolve key codes")
    try:
        keycode = x11.XKeysymToKeycode(display, symbol)
        # シフト無しの面（group 0, level 0）で同じ keysym になるキーだけを受け付ける
        if keycode == 0 or x11.XkbKeycodeToKeysym(display, keycode, 0, 0) != symbol:
