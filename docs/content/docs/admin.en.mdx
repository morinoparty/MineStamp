---
title: Administrator documentation
description: Commands and configuration for administrators
---

## Introduction.

The libraries used by MineStamp (Kotlin, AWS SDK, etc.) are not bundled in the plugin JAR; Paper downloads them automatically from Maven Central on first startup.
The emoji font is also downloaded on first startup, so the server needs internet access the first time it starts.

Generate public and private keys on the server<br />
To run `/stamp generate keyPair`<br />

If there is more than one server, this should be copied. <br />
In addition, S3 is used for multiple servers, so configure S3 settings in config.json on each server.

## List of administrative commands

command: `/minestamp advance <abstract stamp> [time] [size] [particle size] [accuracy]`<br />
example: `/minestamp advance :cucumber: 1 3 1.5 32`<br />
description: Generates more advanced stamps.<br />

command: `/minestamp generate keypair`<br />
description: Generate private and public keys.<br />

command: `/minestamp publish roulette`<br />
description: Generate tickets for roulette.<br />

command: `/minestamp publish unique <stamp>`<br />
example: `/minestamp publish unique :cucumber:`<br />
description: Generate a ticket to activate a specific stamp<br />

command: `/minestamp reload`<br />
description: Reloads the settings. Also, if images etc. are added, they will be reflected by executing this command.<br />

command: `/minestamp purge`<br />
description: Permanently removes emojis that became unrenderable after the font update from all player data and random.json. Removed entries are reported as `deleted <playerName>: <shortCode>`.<br />

## Add language settings

Add a file with the language code in the lang directory. The file name is the language code. The file format is
properties. <br />
example:`zh_CN.properties`, `fr_FR.properties` <br />
defaultSupportedLanguage: `en_US`, `ja_JP` <br />
if you want to add a new language, please create a file with the language code in the lang directory. <br />
original file is [here](https://github.com/Nlkomaru/MineStamp/blob/master/src/main/resources/lang/en_US.properties) <br />

## Configuration file and directory structure

### config.json

Basic configuration. This will always be placed on each server.
The `stamp` section contains the rendering parameters used when summoning stamps, and the `font` section configures the font used for emoji stamps.

```json
{
  "type": "S3",
  "s3Config": {
    "url": "http://minio.example.com:9000/",
    "bucket" : "test",
    "accessKey" : "minio",
    "secretKey" : "minio123"
  },
  "lang" : "en_US",
  "stamp": {
    "second": 3,
    "size": 1.8,
    "particleSize": 1.0,
    "accuracy": 32,
    "waitSecond": 5.0
  },
  "font": {
    "file": "FluentEmojiColor-CBDT.ttf",
    "url": "https://github.com/morinoparty/MineStamp/releases/download/fonts-v1/FluentEmojiColor-CBDT.ttf",
    "sha256": null
  }
}
```

#### 🔤 Emoji font (`font`)

Emoji stamp images are read from an emoji font placed in the `fonts/` directory.
The font is not bundled with the plugin. If the file specified by `file` does not exist in `fonts/`, it is downloaded automatically from `url` on first startup.

| Key | Description |
|-----|-------------|
| `file` | File name of the font under `fonts/`. Path separators are not allowed. |
| `url` | Download source used when the file does not exist. If `null`, no download is performed. |
| `sha256` | If set, the SHA-256 of the downloaded file is verified. |

Only CBDT (color bitmap) emoji fonts are supported. COLRv1 and SVG fonts cannot be used.
Microsoft Fluent Emoji is used by default. For example, to switch to Noto Color Emoji, set the following and run `/minestamp reload`.

```json
"font": {
  "file": "NotoColorEmoji.ttf",
  "url": "https://raw.githubusercontent.com/googlefonts/noto-emoji/v2.051/fonts/NotoColorEmoji.ttf"
}
```

On servers without internet access, place the font file in `fonts/` manually.
If the font cannot be obtained or loaded, emoji stamps are disabled, but image stamps remain available.

### random.json

This is the setting when using roulette tickets. Set the shortCode and the corresponding weight.

```json
{
  ":last-track-button:": 1,
  ":hourglass-not-done:": 1,
  ":man-cook:-light-skin-tone:": 1,
  ":man-detective:-dark-skin-tone:": 1,
  ":person:-medium-skin-tone--blond-hair:": 1,
  ":clapping-hands:-light-skin-tone:": 1,
  ":cricket-game:": 1
}
```

### player-default.json

The list of default stamps every player can use from the start.
The rendering parameters (`second`, etc.) have moved to the `stamp` section of config.json.
Files in the old format are migrated automatically at startup.

```json
{
  "defaultEmoji": [
    ":cucumber:",
    ":thinking-face:",
    ":angry-face:",
    ":sleeping-face:"
  ]
}
```

## For single server

```
└── EmojiStamp/
    ├── fonts/
    │   └── FluentEmojiColor-CBDT.ttf
    ├── lang/
    │   ├── zh_CN.properties
    │   └── fr_FR.properties
    ├── image/
    │   ├── picture-1.png
    │   └── picture-2.jpg
    ├── player/
    │   ├── aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa.json
    │   └── ffffffff-ffff-ffff-ffff-ffffffffffff.json
    ├── config.json
    ├── player-default.json
    ├── random.json
    ├── privateKey
    └── publicKey
```

## For multiple servers

This is a multi-server configuration.

Directory structure on the local server side

```
└── EmojiStamp/
    ├── fonts/
    │   └── FluentEmojiColor-CBDT.ttf
    ├── lang/
    │   ├── zh_CN.properties
    │   └── fr_FR.properties
    ├── config.json
    ├── privateKey
    └── publicKey
```

Directory structure on the s3 side

```
├── image/
│   ├── picture-1.png
│   └── picture-2.jpg
├── player/
│   ├── aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa.json
│   └── ffffffff-ffff-ffff-ffff-ffffffffffff.json
├── random.json
└── player-default.json
```
